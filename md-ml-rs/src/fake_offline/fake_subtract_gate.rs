...
pub struct FakeSubtractGate<S: Spdz2kShare, const N: usize> {
    pub data: FakeGateData<S, N>,
}

impl<S: Spdz2kShare, const N: usize> FakeSubtractGate<S, N> {
    pub fn new(input_x: FakeGateRef<S, N>, input_y: FakeGateRef<S, N>) -> Self {
        ...
        FakeSubtractGate { data }
    }
}

impl<S: Spdz2kShare, const N: usize> FakeGate<S, N> for FakeSubtractGate<S, N> {
    impl_fake_gate_common!(FakeSubtractGate, S, N);
    ...
    fn do_run_offline(&mut self, party: &mut FakeParty<S, N>) {
        ...
        for i in 0..N {
            self.data.lambda_shr[i] = matrix_subtract(&ix_shr[i], &iy_shr[i]);
            self.data.lambda_shr_mac[i] = matrix_subtract(&ix_mac[i], &iy_mac[i]);
        }
